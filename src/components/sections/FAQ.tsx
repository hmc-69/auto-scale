...
export const FAQ = () => {
  ...
  return (
    <section id="faq" className="py-24 bg-nocturnal/20 relative" aria-label="Frequently Asked Questions">
            ...
            return (
              <div 
                ...
                className={`glass-card rounded-2xl overflow-hidden transition-all duration-300 ${
                  isOpen ? 'border-forsythia/30 shadow-[0_0_15px_rgba(255,200,1,0.05)]' : 'border-mint/10 hover:border-mint/30'
                }`}
              >
                <button
                ...
                >
                  <h3 className={`text-lg font-bold pr-8 transition-colors ${isOpen ? 'text-forsythia' : 'text-powder'}`}>
                    {faq.question}
                  </h3>
                  <div className={`shrink-0 w-8 h-8 rounded-full border flex items-center justify-center transition-all duration-300 ${
                    isOpen ? 'border-forsythia text-forsythia rotate-180 bg-forsythia/10' : 'border-mint/20 text-mint'
                  }`}>
